...
def build_dcmn():
    ...
    if os.path.exists(output_eval_file) and config.output_file != 'output_test.txt':
    ...
    num_train_steps = int(len(train_seq_dataset) / config.batch_size / config.gradient_accumulation_steps * config.num_train_epochs)
...
def build_seq2seq(config, hidden_size, no_cuda):
    ...
    decoder = DecoderRNN(len(config.tokenizer.vocab), config.max_seq_length, hidden_size * 2 if bidirectional else hidden_size,
...
def main():
    dcmn, config, train_dataloader, eval_dataloader, dcmn_optimizer, dcmn_scheduler, dcmn_loss_fun = build_dcmn()

    seq2seq, seq_optimizer, seq_scheduler, seq_loss_fun = build_seq2seq(config, 768, config.no_cuda)
    save_file_best = torch.load('./backup/v1/mctest_output/output_test.bin', map_location=torch.device('cuda:2'))
    # seq2seq.load_state_dict(save_file_best['seq_para'])
    dcmn.load_state_dict(save_file_best)
