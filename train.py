...
def train(model, dataloader, optimizer, dcmn_scheduler, loss_fun,
    ...
    nb_steps = 0
    nb_dcmn_steps = 0
    for step, (seq_batches, dcmn_batches) in enumerate(tqdm(dataloader, desc="Iteration")):
...
def valid(dcmn, dataloader, loss_fun, seq2seq, epoch, config):
    ...
    results = []
    seq_srcs_all = []
    for step, (seq_batches, dcmn_batches) in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        src_ids, src_masks = seq_tokenize(seq_srcs, config)
        decoder_outputs, decoder_hidden, ret_dict = seq2seq([src_ids, src_masks], None, 0.0)
